...
class TestMatrixEffectPerformance:
    """Performance tests for Matrix rain effect generation."""
    ...
    def test_matrix_effect_short_duration_performance(self, benchmark, video_generator):
        """Test Matrix effect performance with short duration (5 seconds)."""
        try:
            duration = 5  # 5 seconds
            job_id = "matrix_perf_test_short"

            # Measure total generation time
            result, total_time = benchmark.time_function(
                "matrix_short_total",
                video_generator.generate_matrix_rain,
                job_id=job_id,
                duration=duration,
                loop_seamless=True,
                output_format="png"  # Skip FFmpeg for pure rendering performance
            ...
            print(f"\nMatrix Effect Short Duration Performance:")
            print(f"  Duration: {duration}s")
            print(f"  Generation Time: {total_time:.3f}s")
            print(f"  Ratio: {generation_ratio:.2f}x realtime")
            ...
            # Verify requirement (Matrix effect has stricter requirement than typing)
            assert generation_ratio <= 1.5, f"Matrix generation too slow: {generation_ratio:.2f}x > 1.5x realtime"

        finally:
            # Cleanup
            self._cleanup_test_files(job_id)

    def test_matrix_effect_standard_duration_performance(self, benchmark, video_generator):
        """Test Matrix effect performance with standard duration (15 seconds)."""
        try:
            duration = 15  # Standard 15-second loop
            job_id = "matrix_perf_test_standard"

            # Measure total generation time
            result, total_time = benchmark.time_function(
                "matrix_standard_total",
                video_generator.generate_matrix_rain,
                job_id=job_id,
                duration=duration,
                loop_seamless=True,
            ...
            print(f"\nMatrix Effect Standard Duration Performance:")
            print(f"  Duration: {duration}s")
            print(f"  Generation Time: {total_time:.3f}s")
            print(f"  Ratio: {generation_ratio:.2f}x realtime")
            print(f"  Target: ≤1.5x realtime")
    ...
    def test_matrix_effect_extended_duration_performance(self, benchmark, video_generator):
        """Test Matrix effect performance with extended duration (30 seconds)."""
        try:
            duration = 30  # Extended duration test
            job_id = "matrix_perf_test_extended"

            # Measure total generation time
            result, total_time = benchmark.time_function(
                "matrix_extended_total",
                video_generator.generate_matrix_rain,
                job_id=job_id,
                duration=duration,
                loop_seamless=True,
            ...
            print(f"\nMatrix Effect Extended Duration Performance:")
            print(f"  Duration: {duration}s")
            print(f"  Generation Time: {total_time:.3f}s")
            print(f"  Ratio: {generation_ratio:.2f}x realtime")
            print(f"  Target: ≤1.5x realtime")
    ...
    def test_matrix_effect_bottleneck_analysis(self, video_generator):
        """Perform detailed bottleneck analysis for Matrix effect using the performance profiler."""
        # Reset profiler for clean analysis
        profiler.reset()
        ...
        try:
            ...
            print(f"\nPerforming Matrix effect bottleneck analysis...")
            ...
            # Save profiling data
            profiler.save_report("matrix_effect_bottleneck_analysis.txt")
            profiler.save_json_data("matrix_effect_bottleneck_data.json")

            # Analyze bottlenecks
            analysis = profiler.get_pipeline_analysis()

            print(f"\nMATRIX EFFECT BOTTLENECK ANALYSIS:")
            print(f"  Total Generation Time: {total_time:.3f}s")
            print(f"  Target Duration: {duration}s")
            print(f"  Performance Ratio: {total_time/duration:.2f}x realtime")
            print(f"  Target Requirement: ≤1.5x realtime")
            print(f"  Status: {'PASS' if total_time/duration <= 1.5 else 'FAIL'}")
            ...
            if "operations" in analysis:
                ...
                print(f"\nTOP MATRIX EFFECT BOTTLENECKS:")
                for i, (op_name, stats) in enumerate(sorted_ops[:5]):
                    print(f"  {i+1}. {op_name}: {stats['percentage_of_total']:.1f}% "
                          f"({stats['total_duration']:.3f}s)")

            # Matrix-specific performance recommendations
            print(f"\nMATRIX EFFECT PERFORMANCE RECOMMENDATIONS:")
            if "operations" in analysis:
                main_loop_stats = analysis["operations"].get("main_rendering_loop", {})
                if main_loop_stats.get("percentage_of_total", 0) > 70:
                    print(f"  - Main rendering loop is the primary bottleneck ({main_loop_stats['percentage_of_total']:.1f}%)")
                ...
                if font_load_stats.get("percentage_of_total", 0) > 5:
                    print(f"  - Font loading overhead detected ({font_load_stats['percentage_of_total']:.1f}%)")
                    print(f"  - Consider font caching or preloading for multiple font sizes")

            # Verify performance requirement (stricter for Matrix effect)
            assert total_time/duration <= 1.5, f"Matrix performance requirement not met: {total_time/duration:.2f}x > 1.5x realtime"
    ...
    def test_generate_matrix_performance_report(self, benchmark):
        """Generate and display comprehensive Matrix effect performance report."""
        # This test should run after the other Matrix performance tests
        report = benchmark.generate_report()
        print(f"\n{report}")

        # Save report to file
        report_file = config.OUTPUT_DIR / "matrix_effect_performance_report.txt"
        with open(report_file, 'w') as f:
            f.write(report)

        print(f"\nMatrix effect performance report saved to: {report_file}")
...
class TestLoadTesting:
    ...
    def test_concurrent_matrix_jobs(self, video_generator):
        """Test multiple concurrent Matrix effect jobs."""
        import threading
        import time

        num_jobs = 2  # Fewer Matrix jobs due to higher resource usage
        ...
        def generate_job(job_id):
            try:
                start_time = time.perf_counter()
                video_generator.generate_matrix_rain(
                    job_id=f"load_test_matrix_{job_id}",
                    duration=duration,
                    loop_seamless=True,
                    output_format="png"  # Skip FFmpeg for faster testing
                )
        ...
        # Wait for all jobs to complete
        for thread in threads:
            thread.join(timeout=600)  # 10 minute timeout per job
        ...
        print(f"\nLOAD TEST RESULTS - CONCURRENT MATRIX JOBS:")
        print(f"  Total Jobs: {num_jobs}")
        print(f"  Successful: {len(successful_jobs)}")
        print(f"  Failed: {len(failed_jobs)}")
        print(f"  Total Time: {total_time:.2f}s")
        ...
        # Verify performance is reasonable (Matrix is slower)
        avg_ratio = sum(r['ratio'] for r in successful_jobs) / len(successful_jobs)
        assert avg_ratio <= 10.0, f"Average performance too slow: {avg_ratio:.2f}x > 10.0x realtime"
